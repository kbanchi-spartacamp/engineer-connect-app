    ...
    <div class="container lg:w-3/4 md:w-4/5 w-11/12 mx-auto my-8 px-4 py-4 bg-white shadow-md">
        ...
        <div class="">
            <div class="flex flex-wrap flex-row">
                ...
                @if ($message->send_mentor_id == Auth::guard(MentorConst::GUARD)->user()->id)
                <div class="w-1/4"></div>
                <div class="w-3/4">
                ...
                <div class="w-3/4">
                    <div class="flex flex-col sm:flex-row items-center text-center my-4">
                        <img class="h-8 w-8 rounded-full object-cover" src="{{ $mentor->profile_photo_url }}"
                            alt="{{ $mentor->name }}" />
                        <p class="text-gray-700 text-sm text-left m-2">
                            {{ $message->created_at }}
                        </p>
            ...
            <form action="{{ route('mentors.messages.store', $mentor) }}" method="POST"
                onsubmit="checkDoubleSubmit(document.getElementById('sendBtn'))" class="rounded pt-3 pb-8 mb-4 ">
                @csrf
